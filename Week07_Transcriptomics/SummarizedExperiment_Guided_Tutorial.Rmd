...
Use the dim() function to find the dimensions of the colData, rowRanges@elementMetadata, and unstranded dataframes. Record them below:
  ...
  ~ rowRanges@elementMetadata: 60660 row, 10 
  # row vs column = genes vs patient 
  ~ rna_se@assays@data$unstranded: 60660, 1231
  #specifically looking at this asset 
...
```{r Exercise 2.1}
# write code here
# in the r syntax, last column also counts
...
rna_clinical -<-clinical[age_NA_mask,]
...
```{r Exercise 3.1}
# write code here
head(rowRanges@elementMetadata)
```
...
rna_counts[1:15, 1:5] #first 1-15 rows and first 1-5 columns of patients 
```
...
#rownames(rna_genes) <- rna_genes$gene_name wrong!
