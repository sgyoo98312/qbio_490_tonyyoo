...
## ---- 0. Drop the two metadata rows that came with the CSV ----
clinic <- clinic[-c(1, 2), , drop = FALSE]
row.names(clinic) <- NULL

## ---- 1. Convert types we need ----
clinic$age_at_diagnosis     <- to_num(clinic$age_at_diagnosis)
clinic$last_contact_days_to <- to_num(clinic$last_contact_days_to)
clinic$death_days_to        <- to_num(clinic$death_days_to)
clinic$vital_status         <- toupper(trimws(clinic$vital_status))  # "ALIVE"/"DEAD"

## ---- 2. Survival time and event flag ----
clinic$survival_time <- ifelse(!is.na(clinic$death_days_to),
                               clinic$death_days_to,
                               clinic$last_contact_days_to)
clinic$death_event   <- clinic$vital_status == "DEAD"

## Remove rows we can’t analyze (NA or negative time, NA age/event)
clinic <- subset(
  clinic,
  !is.na(survival_time) & survival_time >= 0 &
  !is.na(age_at_diagnosis) & !is.na(death_event)
)

## ---- 3. Age categories (<=35: Young; 35–<50: Middle; >=50: Old) ----
clinic$age_category <- ifelse(clinic$age_at_diagnosis <= 35, "Young",
                        ifelse(clinic$age_at_diagnosis < 50,  "Middle", "Old"))
clinic$age_category <- factor(clinic$age_category, levels = c("Young","Middle","Old"))

## ---- 4. KM fit + p-value + 15-year survival ----
library(survival); library(survminer); library(ggplot2)

fit <- survfit(Surv(survival_time, death_event) ~ age_category, data = clinic)

## Log-rank test p-value
logrank <- survdiff(Surv(survival_time, death_event) ~ age_category, data = clinic)
pval <- pchisq(logrank$chisq, df = nlevels(clinic$age_category) - 1, lower.tail = FALSE)
pval

## 15-year survival (≈ 15*365 days)
## EX 6.4
t15 <- 15 * 365  # 15 years in days

# Make sure you're using the same object name you fit in EX 6.2:
# fit_object <- survfit(Surv(survival_time, death_event) ~ age_category, data = clinic)

sf <- summary(fit_object, times = t15, extend = TRUE)

km15_tbl <- data.frame(
  age_group = sub("^age_category=", "", as.character(sf$strata)),
  surv_15y  = round(sf$surv, 3),
...
What is the p‑value? Is it statistically significant, and what does that mean?
